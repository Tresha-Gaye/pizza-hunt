...
const pizzaController = {
    ...
    // get one pizza by id
    getPizzaById({ params }, res) {      // we destructure the `params` out of the Express.js `req` object since that's all we need
    ...
    updatePizza({ params, body }, res) {
        Pizza.findOneAndUpdate({ _id: params.id }, body, { new: true })  // this method allows mongoose to find a single document and update it. new:true causes it to return the updated document instead of the original
        .then(dbPizzaData => {                                           // Mongoose & MongoDB also have .updateOne() & .updateMany() that updates documents without returning them
            if(!dbPizzaData) {
                res.status(404).json({ message: 'No pizza found with this id!' });
                return;
            }
